...
export const PlantGrowth = () => {
  ...
  return (
    <section className="py-xxl px-lg bg-gradient-to-br from-primary/5 via-accent/10 to-primary/5">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="lg:hidden space-y-xl">
          {stages.map((item, index) => (
            <div key={index} className="flex gap-lg items-start">
              <div className="flex-shrink-0 relative">
                <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold border-4 border-background shadow-md">
                  {item.stage}
                </div>
                {index < stages.length - 1 && (
                  <div className="absolute top-12 left-1/2 -translate-x-1/2 w-1 h-24 bg-border" />
                )}
              </div>
              
              <div className="flex-1 bg-card rounded-xl p-lg border border-border shadow-sm">
                <div className="flex flex-col sm:flex-row gap-md items-center">
                  <div className="flex-shrink-0">
                    <PlantAvatar stage={item.stage} className="w-32 h-24" />
                  </div>
        ...
        <div className="mt-xxl text-center bg-card rounded-2xl p-xl border border-border shadow-sm max-w-2xl mx-auto">
          <p className="text-lg text-foreground font-medium mb-sm">
